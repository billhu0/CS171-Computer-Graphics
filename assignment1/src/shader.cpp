...
void Shader::initWithCode(const std::string &vs, const std::string &fs, const std::string &gs) {
    // get the source code (char[]) of VertexShader, FragShader and GeometryShader
    const GLchar *vShaderCode = vs.c_str();
    const GLchar *fShaderCode = fs.c_str();
    const GLchar *gShaderCode = gs.c_str();
    ...
    // Now 3 shaders have been compiled. We need to link them to form a Shader Program Object (着色器程序对象)
    id = glCreateProgram();  // Create Shader Program. 'id' is the id reference to the object.
    glAttachShader(id, VertexShader);
    glAttachShader(id, FragmentShader);
    glAttachShader(id, GeometryShader);  // attach compiled shader to this program object
    glLinkProgram(id);
    glGetProgramiv(id, GL_LINK_STATUS, &success);  // check if compilation succeeds
    if (!success) {
        glGetProgramInfoLog(id, 512, nullptr, infoLog);
